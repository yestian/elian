...
class Member extends Common{
	...
	public function lst($fld='id',$way='asc'){
		...
		//充值
		$sub1=db('member_pay')->field('aid,sum(money) as allpay')->group('aid')->buildSql();
		//消费
		$sub2=db('member_buy')->field('aid,sum(expense) as allbuy')->group('aid')->buildSql();
		//下级代理商数量，同表关联
		$sub3=db('member')->field('upid,count(id) as upidsum')->group('upid')->buildSql();
		//会员数量
...
//---------------------------------------------
	...
	public function add(){
		...
		if(request()->isPost()){
			...
			//------------------------------插入数据
			$agent=MemberModel::create($data1);
			$agent->save($data1);
	...
	public function edit(){
		...
		if(request()->isPost()){
			...
			//实例化
			$agent=MemberModel::get(input('id'));
			//图片处理---------------------------------------------
