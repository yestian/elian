...
use app\member\model\MemberArticle as MemberArticleModel;
use app\member\model\MemberCate as MemberCateModel;
use app\member\model\Member as MemberModel;

class MemberArticle extends Common{
	public function lst(){
		...
		//连表查询，使用栏目名称
		$artres=db('member_article')->field('a.*,b.catename')->alias('a')->join('cate b','a.cateid=b.id')->paginate(10);
	...
	public function add(){
		...
		if(request()->isPost()){
			...
			$article=new MemberArticleModel();
...
public function edit(){
	$menumodel=new MemberModel;
        ...
        $cate=new MemberCateModel();
        $cateres=$cate->catetree();
        $arts=db('member_article')->where(array('id'=>input('id')))->find();
        $this->assign(array(
            'cateres'=>$cateres,
            'arts'=>$arts,
            ));
	...
	public function del(){
		$res=MemberArticleModel::destroy(input('id'));
		if($res){
